	...
	if ( !class_exists('QUERY') ) {
		class QUERY {
			public function load_user_object($user_id) {
				...
				$obj = $hm_db->select($query);
				
				if ( !$obj ) {
					return "No user found1";
				}
			...
			public function get_status_objects($user_id) {
				...
				$status_objects = $hm_db->select($query);
